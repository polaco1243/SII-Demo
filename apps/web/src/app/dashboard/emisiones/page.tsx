...
async function subirCsv(formData: FormData) {
  ...
  if (!siiCredentialId || !archivo || archivo.size === 0) {
    redirect("/dashboard/emisiones?error=faltan_datos");
  }
  ...
  try {
    ...
    redirect("/dashboard/emisiones?error=" + encodeURIComponent("el archivo no se pudo leer como CSV"));
  }
  ...
  const batchId = await withUser(userId, async (tx) => {
    ...
    if (errorValidacion) {
      redirect("/dashboard/emisiones?error=" + encodeURIComponent(errorValidacion));
    }
...
export default async function EmisionesPage({
  ...
  return (
    <div className="fade-in mx-auto max-w-7xl p-4 md:p-8">
      ...
      <h1 className="mb-6 border-b border-border pb-5 text-page">Emisiones</h1>
